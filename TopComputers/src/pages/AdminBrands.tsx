          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {brands.map((brand) => (
              <motion.tr
                ...
                className="hover:bg-gradient-to-r hover:from-orange-50 hover:to-red-50 dark:hover:from-gray-700 dark:hover:to-gray-600 transition-all duration-300"
              >
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                    {brand.logo ? (
                      ...
                      <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-orange-100 to-red-100 dark:from-orange-900/30 dark:to-red-900/30 flex items-center justify-center mr-4 border-2 border-orange-200 dark:border-orange-700">
...
interface BrandFormProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSave: (brandData: Omit<Brand, 'id' | 'createdAt' | 'updatedAt'>) => void;
}

function BrandForm({ brand, isOpen, onClose, onSave }: BrandFormProps) {
  ...
  // Update form data when brand changes (for editing)
  useEffect(() => {
    if (brand) {
      setFormData({
        name: brand.name || '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setFormData({ name: '', slug: '', logo: '', active: true });
    onClose();
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <MediaSelector
              ...
              onSelect={(imageUrl) => setFormData({ ...formData, logo: imageUrl || '' })}
              label="Brand Logo"
              required={true}
            />
          </div>
...
export default function AdminBrands() {
  const { brands, loadingBrands, createBrand, updateBrand, deleteBrand } = useCatalog();
  ...
  const handleSave = async (brandData: Omit<Brand, 'id' | 'createdAt' | 'updatedAt'>) => {
    if (editingBrand) {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div className="bg-gradient-to-r from-orange-600 to-red-600 dark:from-gray-900 dark:to-gray-700 rounded-2xl p-8 shadow-2xl">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
          <div>
            ...
            <p className="text-orange-100 dark:text-gray-300 text-lg">
              Manage product brands ({filteredBrands.length} of {brands.length} brands)
            </p>
          </div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-12 pr-4 py-4 border-2 border-gray-300 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:border-orange-500 dark:focus:border-orange-400 focus:ring-2 focus:ring-orange-200 dark:focus:ring-orange-800 transition-all duration-200 font-medium text-lg"
        />
      </div>
      ...
      {filteredBrands.length > 0 ? (
        ...
        <div className="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-2xl shadow-2xl p-16 text-center border border-gray-200 dark:border-gray-700">
          <Tag className="h-20 w-20 text-gray-400 mx-auto mb-6" />
          ...
          {!searchTerm && (
            <button
              onClick={() => setShowForm(true)}
              className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-700 hover:to-red-700 text-white font-bold rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105"
            >
