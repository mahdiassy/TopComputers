...
export function TransactionProvider({ children }: TransactionProviderProps) {
  ...
  useEffect(() => {
    ...
    fetchUsernames();
  }, [currentUser, userData, transactions, activityLogs, getUserData]);

  // Fetch transactions with optimized queries and pagination
  useEffect(() => {
    ...
    // COST OPTIMIZATION: Query only transactions for current account
    // Add compound indexes for these queries in firestore.indexes.json
    const q = query(
      transactionsRef,
      where('accountId', '==', currentAccount.id),
      orderBy('date', 'desc'),
      limit(100) // PAGINATION: Load only last 100 transactions initially
    );
  ...
  // Fetch activity logs with optimized query
  useEffect(() => {
    if (!currentUser || !currentAccount) return;
    
    const logsRef = collection(db, 'activityLogs');
    // COST OPTIMIZATION: Query only logs for current account with pagination
    const q = query(
      logsRef, 
      where('accountId', '==', currentAccount.id),
      orderBy('timestamp', 'desc'),
      limit(50) // PAGINATION: Load only last 50 activity logs
    );
  ...
  async function addTransaction(transaction: TransactionInput) {
    ...
    try {
      ...
      // Only add budgetCategoryId to activity log if it exists
      if (transaction.budgetCategoryId) {
        activityLogData.transactionDetails.budgetCategoryId = transaction.budgetCategoryId;
      }
      
      await addDoc(logsRef, activityLogData);
